...
#plt.show()
pred = results.get_prediction(start=pd.to_datetime('2020-06-09'), dynamic=False)
pred_ci = pred.conf_int()
ax = data['1980':].plot(label='observed')
...
data_truth = data['2020-06-09':]['tmax']

# Compute the mean square error
mse = ((data_forecasted - data_truth) ** 2).mean()
print('The Mean Squared Error of our forecasts is {}'.format(round(mse, 2)))

pred_dynamic = results.get_prediction(start=pd.to_datetime('2020-06-09'), dynamic=True, full_results=True)
...
ax.fill_betweenx(ax.get_ylim(), pd.to_datetime('2020-06-09'), data['tmax'].index[-1],
                 alpha=.1, zorder=-1)
